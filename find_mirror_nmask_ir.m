...
% c_semifinal_corners=[];
% final_corners=[];
X=imadjust(uint8(img1));
%Y = imgaussfilt(X, 4);
X=imsharpen(X,'Radius',4,'Amount',2);




level = graythresh(X);        %Umbralizamos
mask1 = imbinarize(X,level);   
%figure;
% subplot(121)
% imshow(X)
% subplot(122)
...
mask_w_w = bwareaopen(mask_w_w,25);  %Retirando manchas blancas
[L,numBlack] = bwlabel(~mask_w_w); %Etiquetamos los agrupamientos de pixeles.
mask= ~bwareaopen(L,25);   %Retirando manchas negras
