...
% Cargamos digital image with trad proj
pc_dig2 = pcread([direc 'Digital Features 2 UTB\obj_vis_ir_0' aux '.ply']);
subplot(133)
pcshow(pc_dig2);
xlabel('x (mm)')
ylabel('y (mm)')
...
% Digital features II
x_dig2=pc_dig2.Location(:,1);
y_dig2=pc_dig2.Location(:,2);
z_dig2=pc_dig2.Location(:,3);
t_dig2=pc_dig2.Intensity;
%[X_dig,Y_dig]=meshgrid([min(x_dig):0.05:max(x_dig)], [min(y_dig):0.05:max(y_dig)]);

[X_dig2,Y_dig2]=meshgrid(linspace(min(x_dig2),max(x_dig2),1280), linspace(min(y_dig2),max(y_dig2),1024));



z_est_dig2=griddata(x_dig2,y_dig2,z_dig2,X_dig2,Y_dig2,'cubic');
t_est_dig2=griddata(x_dig2,y_dig2,t_dig2,X_dig2,Y_dig2,'cubic');
z_est_dig2(z_est_dig2>max(z_dig2))=NaN;
t_est_dig2(t_est_dig2>max(t_dig2))=NaN;
z_est_dig2(z_est_dig2<min(z_dig2))=NaN;
t_est_dig2(t_est_dig2<min(t_dig2))=NaN;
...
% Visualizacion 3D
figure;
s = surf(-X_dig2,Y_dig2,-z_est_dig2,'FaceColor', 'interp',...
            'EdgeColor', 'none',...
            'FaceLighting', 'phong');
...
disp('Cantidad de datos NaN:')
sum(sum(isnan(z_est_dig2))) % Ojo con esto
sum(sum(isnan(t_est_dig2))) % Ojo con esto
%%
...
profile=-z_est_dig(n_profile,largo_profile);
...
p1 = polyfit(1:length(profile_z_dig_sin_primary),profile_z_dig_sin_primary,3); 
...
%% Vista de perfiles horizontales - Digital Features 2
% Digital Features

n_profile=510;

figure;
imagesc(z_est_dig2,[360 430])
yline(n_profile,'--k')
colorbar
...
tam=length(X_dig2(n_profile,:));
largo_profile=120:tam-20;
figure(6);
subplot(211)
plot(z_est_dig2(n_profile,largo_profile))
xlabel('X (mm)')
ylabel('Z (mm)')
subplot(212)
plot(t_est_dig2(n_profile,largo_profile))
xlabel('X (mm)')
ylabel('t (°C)')


% Vista de perfiles en la misma grafica sin perfil primario
profile=-z_est_dig2(n_profile,largo_profile);
profile=profile(~isnan(profile));
profile_z_dig2=profile;
X_dig2_sin_nan=X_dig2(n_profile,:);
X_dig2_sin_nan=X_dig2_sin_nan(~isnan(profile));


p = polyfit(1:length(profile),profile,1); 
f = polyval(p,1:length(profile)); 
profile_z_dig2_sin_primary=profile-f;
% figure;
% plot(f)
% hold on
% plot(profile)
...
%quitamos perfil primario
%Buscamos el perfil termico
profile_t_dig2=t_est_dig2(n_profile,largo_profile);
profile_t_dig2=profile_t_dig2(~isnan(profile));

size(profile_z_dig2_sin_primary)
size(profile_t_dig2)
figure(7);
subplot(312)
yyaxis left
offset=-min(profile_z_dig2_sin_primary);
offset=0;
plot((profile_z_dig2_sin_primary+1*offset),'b')
ylabel('Z (mm)')
% yt = get(gca, 'YTick');
% set(gca, 'YTick',yt, 'ZTickLabel',fliplr(yt))
%ylim([-1 1])
...
%quitamos perfil secundario
p1 = polyfit(1:length(profile_z_dig2_sin_primary),profile_z_dig2_sin_primary,3); 
f1 = polyval(p1,1:length(profile_z_dig2_sin_primary)); 
figure(8);
subplot(312)
yyaxis left
plot((profile_z_dig2_sin_primary-f1))
ylabel('Z (mm)')
%ylim([-1 1])
yyaxis right
plot(profile_t_dig2)
...
aux=(profile_z_dig2_sin_primary-f1);
profile_final_z_dig2=aux;
tam_aux=1:length(aux);
%tam_aux=550:700;
%tam_aux=570:590
...
%title('Digital')
grid on
legend('Z  by Digital features 1','Z  by Digital features 2','Z  by Traditional','t by Digital features 1','t by Digital features 2','t by Traditional')
grid minor
%%
