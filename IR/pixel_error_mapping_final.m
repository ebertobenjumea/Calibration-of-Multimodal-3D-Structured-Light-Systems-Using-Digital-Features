...
mask_dig=load([direc 'Digital Features 1 UTB\obj_vis_ir_03.mat'],'mask_final');
...
mask_trad=load([direc 'Two Stages 2\obj_vis_ir_03.mat'],'mask_final');
...
mask_w_w = bwareaopen(mask_w_w,4000);  %Retirando manchas blancas
...
%  figure; 
%  imshow(a,[])
% [puntos, radii, metric] = imfindcircles(BW,[40 100],'ObjectPolarity','bright',Sensitivity=0.9);
% figure;
% imagesc(BW)
% hold on
...
figure; imshow(a)
[mask_w_w,numWhite] = bwlabel(a);
mask_w_w = bwareaopen(mask_w_w,4500);  %Retirando manchas blancas
figure; imshow(mask_w_w)
...
radii_d = diameters/2;
hold on
viscircles(centers_t_dig,radii_d,'LineStyle','--','LineWidth',1,'Color','Black')
...
%%
[idx, dist] = knnsearch(centers_t_trad, centers_t_dig);

% idx contiene los índices de los puntos más cercanos en centers_t_trad
% dist contiene las distancias correspondientes

% Ordenar centers_t_dig según las distancias más cercanas
sorted_centers_t_dig = centers_t_dig(idx, :);

% Mostrar los puntos ordenados
figure;
plot(sorted_centers_t_dig(:, 1), sorted_centers_t_dig(:, 2), '+k');
title('Puntos de centers\_t\_dig ordenados por distancia a centers\_t\_trad');
xlabel('X');
ylabel('Y');
...
for i=1:size(centers_t_trad,1)
    ...
    plot(sorted_centers_t_dig(i,1),sorted_centers_t_dig(i,2),'xk')
    %rms_t=norm(centers_t-centers_dig)
    pause(0.1);
end
rms_t=(sum(vecnorm(centers_t_trad-sorted_centers_t_dig,2,2)))/size(centers_t_trad,1);


errormapt=NaN.*ones(size(t_est_trad_copy));
error_pixel=vecnorm(centers_t_trad-sorted_centers_t_dig,2,2);
for i=1:size(centers_t_trad,1)
