...
clc
clear all
direc='D:\R3D\Adquisiciones\Experimento 45\Objeto\Objeto final 3\SL\';
aux=num2str(5);
% Cargamos digital features image
pc_dig = pcread([direc 'Digital Features 2 UTB\obj_vis_ir_0' aux '.ply']);
figure;
pcshow(pc_dig);
xlabel('x (mm)')
ylabel('y (mm)')
...
pc_trad = pcread([direc 'Two Stages\obj_vis_ir_0' aux '.ply']);
figure;
pcshow(pc_trad);
xlabel('x (mm)')
ylabel('y (mm)')
...
f1 = polyval(p1,1:length(profile_z_trad_sin_primary)); 
figure(8);
subplot(212)
yyaxis left
plot(-(profile_z_trad_sin_primary-f1))
ylabel('Z (mm)')
%ylim([-1 1])
yyaxis right
plot(profile_t_trad)
...
figure;
%yyaxis left
plot(-z_est_dig(n_profile,largo_profile)-5.6,'b')
hold on
